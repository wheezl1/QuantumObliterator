...
Requires the .NET SDK (10.x is what this was built with) and a local Valheim install.

```sh
dotnet build -c Release
```

Paths are set by two MSBuild properties, overridable from the environment:

- `VALHEIM_MANAGED` — the game's `valheim_Data/Managed` directory.
- `GALE_BEPINEX` — a Gale profile's `BepInEx` directory, used to reference BepInEx, Harmony
  and Jotunn from disk rather than from NuGet, so the build always matches what the game
  loads.

Game assemblies are referenced in place and never redistributed. `assembly_valheim` is
publicized at build time via `BepInEx.AssemblyPublicizer.MSBuild`; no publicized DLL is
checked in.

Output is a single file: `bin/Release/QuantumObliterator.dll`.

## Installing

Use a mod manager — Gale, r2modman or the Thunderstore app — and install
**QuantumObliterator**. It requires [BepInExPack Valheim](https://thunderstore.io/c/valheim/p/denikson/BepInExPack_Valheim/)
and [Jotunn](https://thunderstore.io/c/valheim/p/ValheimModding/Jotunn/), which the manager
pulls in for you. Everyone on the server needs it, including the server itself.

### From source

Copy the built DLL into a BepInEx profile alongside Jotunn:

```sh
install -D bin/Release/QuantumObliterator.dll \
  "<profile>/BepInEx/plugins/QuantumObliterator/QuantumObliterator.dll"
```

Then launch through that profile's `start_game_bepinex.sh`. Launching the game from the
Steam directory does **not** load mods — that folder ships doorstop but has no `BepInEx/`
tree.
